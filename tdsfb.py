...
class Facebook_Api(object):
    ...
    def reaction(self, id, type):
        ...
        data = {
            'av': self.actor_id,
            '__usid': r'6-Tsftgotwhb2nus:Psfgosvgerpwk:0-Asfgotw11gc1if-RV=6:F=',
            '__aaid': '0',
            '__user': self.actor_id,
            '__a': '1',
            '__req': '2c',
...
def Main():
    ...
    # Đã loại bỏ hoàn toàn việc đọc/ghi file Cookie_FB.txt
    listck = Nhap_Cookie()
    ...
    while True:
        if len(listck) == 0:
            print(f'{red}Hết Cookie Trong List, Tiến Hành Nhập Mới')
            listck = Nhap_Cookie()
        for ck in listck:
            nhiemvu = listnv[0]
