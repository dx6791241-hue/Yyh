...
class Zefoy:
	...
	def find_video(self):
		...
		while True:
			...
			if 'Session expired. Please re-login' in self.video_info: print('[Nguyên]</> Phiên hết hạn. Đang đăng nhập lại...');self.send_captcha(); return
				...
				elif lamtilo >= 1000: print('[HECTORCLIENT]</> Your IP was banned')
				...
				while time.time()-2<_+lamtilo:
					t-=1
					print("[HECTORCLIENT]</> Vui lòng chờ: {0} ".format(t)+"giây .", end="\r")
					time.sleep(1)
	...
	def use_service(self):
		...
		if 'Session expired. Please re-login' in res: print('[HECTORCLIENT]</> Phiên hết hạn. Đang đăng nhập lại...');self.send_captcha(); return ""
		elif 'Too many requests. Please slow' in res: time.sleep(3)
		elif 'service is currently not working' in res: return ('[HECTORCLIENT]</> Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.')
	...
	def get_video_id(self, url_ = None, set_url=True):
		...
		if request.text == '': print('[HECTORCLIENT]</> Link video không hợ lệ'); return False
	...
	def update_name(self):
		while True:
			try:
				...
				self.text = f"[HECTORCLIENT]</> | Hearts: {like_info['likeCount']} "
			except: pass
			time.sleep(5)
trang = "\033[1;37m"
xanh_la = "\033[1;32m"
...
if 1:
    print(f"{xanh_la}[HECTORCLIENT]</> Key đúng mời bạn vô tool")
    logo()
    Z = Zefoy()
    ...
    print(f"{do}[HECTORCLIENT]</> Key sai vui lòng vượt lại")
    exit()

while True:
	try: 
		if 'Service is currently not working, try again later' in str(Z.use_service()): print(f'{do}[CLIENT]</>Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.');time.sleep(5)
