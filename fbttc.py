...
def Delay(value):
    while not(value <= 1):
        value -= 0.123
        print(f'''{v}[{xanh}HECTORVNX{v}] [{xanh}DELAY{v}] [{xanh}{str(value)[0:6]}{v}] [{vang}Obii   {v}]''', '           ', end = '\r')
        sleep(0.02)
        print(f'''{v}[{xanh}HECTORVNX{v}] [{xanh}DELAY{v}] [{xanh}{str(value)[0:6]}{v}] [ {vang}Obii   {v}]''', '           ', end = '\r')
        sleep(0.02)
        print(f'''{v}[{xanh}HECTORVNX{v}] [{xanh}DELAY{v}] [{xanh}{str(value)[0:6]}{v}] [  {vang}Obii {v}]''', '            ', end = '\r')
...
os.system("cls" if os.name == "nt" else "clear")            
banner()
addcookie() # Đã gỡ bỏ tính năng đọc/ghi file json cookie ở đây
...
while True:
    if len(listCookie) == 0:
        ...
        addcookie() # Đã gỡ bỏ tính năng ghi file json cookie ở đây
                ...
                if "idpost" in getjob.text or "idfb" in getjob.text:
                    ...
                    for x in getjob.json():
                        ...
                        if nhanxu.get('status') == 'success':
                            ...
                            print(f'{do}[ \033[1;36m{stt}{do} ] {do}[ {vang}HECTORVN{do} ][ {xanh}{timejob}{do} ][ {vang}{type.upper()}{do} ][ {trang}{id_}{do} ][ {vang}{msg}{do} ][ {luc}{str(format(int(xu),","))} {do}]')
                            if stt % 10 == 0:
